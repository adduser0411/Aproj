...
class ACmix(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_att=7, head=4, kernel_conv=3, stride=1, dilation=1):
        ...
        # 修改输入通道数为x1的通道数(128)和x2的通道数(320)之和
        ...
        self.conv3 = nn.Conv2d(in_planes*2, out_planes, kernel_size=1)   #todo
    ...
    def forward(self, x1, x2):
        ...
        # 按照要求分别处理x1和x2
        q = self.conv1(x1)  # x1生成q
        k = self.conv2(x2)  # x2生成k
        x = torch.cat([x1, x2], dim=1)  # 拼接x1和x2
        v = self.conv3(x)  # 拼接后的特征生成v
        ...
        pe = self.conv_p(position(h, w, x1.is_cuda))
        # 为自注意力机制准备q, k, v
        # 修改这部分代码
...
if __name__ == "__main__":
    # 输入16, 128, 44, 44
    x=torch.randn(16, 128, 44, 44)
    in_planes = 128
    out_planes = 128
    acmix = ACmix(in_planes, out_planes)
    x1 = torch.randn(1, in_planes, 32, 32)